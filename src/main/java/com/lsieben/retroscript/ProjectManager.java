    ...
    private static void createProject(VnesProject project) {
        ...
        if (!projectDir.exists()) {
            if (!projectDir.mkdirs()) {
                com.lsieben.retroscript.logger.Logger.writeError("Could not create project directory");
            }
            ...
            if (!projectSourceDir.mkdirs()) {
                com.lsieben.retroscript.logger.Logger.writeError("Could not create project source directory");
            }
            try {
                ...
                com.lsieben.retroscript.logger.Logger.writeError("Could not create project definition file");
            }
            ...
            for(String dirPath : templateDirs) {
                ...
                if (!dir.mkdirs()) {
                    com.lsieben.retroscript.logger.Logger.writeError("Could not create project template folder '" + dir.getPath() + "'");
                }
            }
        ...
        if (definitionFile.exists() && definitionFile.canRead()) {
            try {
                ...
                try {
                    ...
                    com.lsieben.retroscript.logger.Logger.writeError("invalid project definition format.");
                }
            } catch(IOException ex) {
                com.lsieben.retroscript.logger.Logger.writeError("Cannot read definition file '" + path + "':" +  ex.getMessage());
            }
        } else {
            com.lsieben.retroscript.logger.Logger.writeError("Definition file '" + path + "' does not exists or is not readable.");
        }
        return null;
    }
